sap.ui.core.mvc.Controller.extend("training.courses.management.view.Administration", {
	...
	_subscribeForCustomEvents : function() {
		sap.ui.getCore().getEventBus().subscribe("deleteFinished", "completed", function() {
			this.refresh();
			this.getView().setBusy(false);
		}, this);

		sap.ui.getCore().getEventBus().subscribe("postFinished", "completed", function() {
			this.refresh();
			this.getView().setBusy(false);
		}, this);

		sap.ui.getCore().getEventBus().subscribe("putFinished", "completed", function() {
			this.refresh();
			this.getView().setBusy(false);
		}, this);
	},
