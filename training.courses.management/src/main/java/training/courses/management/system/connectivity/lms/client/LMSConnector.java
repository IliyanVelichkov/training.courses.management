...
public class LMSConnector implements Connector {
	...
	public LMSConnector(AccessTokenManager accessTokenManager) {
		...
		this.url = PasswordsKeeperFactory.INSTANCE.getPasswordsKeeper().getPassword(PasswordAlias.LMS_URL.getName());
	...
	private String formatResourcePath(String resourcePath) {
		return resourcePath.startsWith("/") ? resourcePath : resourcePath.substring(1); //$NON-NLS-1$
	}

	private ServiceHttpResponse execute(HttpRequestBase request) throws ServiceException {
		HttpResponse response;
		try {
			request.addHeader(createBearerHeader());
			response = httpClient.execute(request);
			validateResponse(response);
			return buildResponse(response);
		} catch (AccessTokenGenerationException ex) {
			return handleError("Failed to generate access token for request to url " + request.getURI(), ex); //$NON-NLS-1$
	...
	private Header createBearerHeader() throws AccessTokenGenerationException {
